...
clear
% Create a file path 
file_path = '/MATLAB Drive/filtered_sine_wave.txt';
...
s_wave= sin(2*pi*t*freq); 

% Open file and read the data
fileID = fopen(file_path, 'r');
hex_data = textscan(fileID, '%s');  % Her satırdaki hex verilerini okuyun
fclose(fileID);


hex_values = hex_data{1}; 

% Change hex values to numbers
dec_values = hexsingle2num(hex_values);

% Plot the filtered sine Wave and Orginal sine Wave
plot(dec_values,'DisplayName','filtered') 
hold 
plot(s_wave(1:400),'DisplayName','Orginal')
